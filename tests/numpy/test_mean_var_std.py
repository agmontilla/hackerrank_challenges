...
class TestMeanVarStd:
    ...
    def test_mean_var_std(self) -> None:
        ...
        data = [[1, 2], [3, 4]]
        result = mean_var_std(data)
        assert all(np.equal(result.mean, np.array([1.5, 3.5])))
        assert all(np.equal(result.var, np.array([1.0, 1.0])))
        assert result.std == 1.11803398875
